...
{#snippet AppleLogo(className?: string)}
    <svg
        fill="none"
        viewBox="0 0 270 333"
...
{#snippet AppleChip(className?: string)}
    <svg
        enable-background="new 0 0 132 92"
        viewBox="0 0 132 92"
        xmlns="http://www.w3.org/2000/svg"
        class={className}
    >
...
{#snippet MastercardLogo(className: string)}
    <svg
        fill="none"
        viewBox="0 0 431 319"
        xmlns="http://www.w3.org/2000/svg"
...
<CreditCard.Root>
    <CreditCard.Flipper>
        <CreditCard.Front class="bg-[#F2F2F2] text-[#909090]" safeArea={24}>
            <CreditCard.Logo class="absolute top-0 right-auto left-0 size-1/8">
                {@render AppleLogo()}
            </CreditCard.Logo>
            <CreditCard.Chip class="right-1 left-auto w-1/5">
                {@render AppleChip()}
            </CreditCard.Chip>
            <CreditCard.Name class="-translate-y-1/2 absolute top-1/2 mt-4">
                John R. Doe
            </CreditCard.Name>
        </CreditCard.Front>
        <CreditCard.Back class="bg-[#F2F2F2] text-[#909090]" safeArea={0}>
            <CreditCard.ServiceProvider
                class="top-6 right-6 max-h-1/4 max-w-1/4"
                type="Mastercard"
            >
                {@render MastercardLogo("w-full")}
            </CreditCard.ServiceProvider>
            <CreditCard.MagStripe class="top-auto bottom-0 bg-[#BEBEC0]" />
        </CreditCard.Back>
    </CreditCard.Flipper>
</CreditCard.Root>
