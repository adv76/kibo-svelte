<script lang="ts">
    import * as CreditCard from "$lib/registry/components/credit-card";
</script>

{#snippet ChaseLogo(className: string)}
    <svg
...
{#snippet ChaseMark(className: string)}
    <svg
        fill="none"
        viewBox="0 0 465 465"
        xmlns="http://www.w3.org/2000/svg"
...
<CreditCard.Root>
    <CreditCard.Flipper>
        <CreditCard.Front class="bg-[#063573]">
            {@render ChaseLogo("absolute top-0 left-0 h-1/12")}
            <CreditCard.Logo>
                {@render ChaseMark("text-[#0e72d1]")}
            </CreditCard.Logo>
            <CreditCard.Chip />
            <CreditCard.ServiceProvider
                class="brightness-0 invert"
                format="logo"
                type="Visa"
            />
            <CreditCard.Name class="absolute bottom-0 left-0">
                John R. Doe
            </CreditCard.Name>
        </CreditCard.Front>
        <CreditCard.Back class="bg-[#063573]">
            <CreditCard.MagStripe />
            <CreditCard.Number class="absolute bottom-0 left-0">
                0123 4567 8901 2345
            </CreditCard.Number>
            <div class="-translate-y-1/2 absolute top-1/2 flex gap-4">
                <CreditCard.Expiry>01/24</CreditCard.Expiry>
